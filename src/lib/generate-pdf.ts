...
import { TBook, TBookPageDimensionsWithBleed, TBookPageDimensionsWithoutBleed } from '@/types/ebook';
...
export const handleCreatePDF = async (pages: TPage[], book: TBook) => {
  try {
    ...
    const dimensions = book.book_type === 'hardcover'
    ...
    pdf.save(`${book.title}_${book.book_type}_${book.size}_${book.paper_color}.pdf`);
  } catch (error) {
